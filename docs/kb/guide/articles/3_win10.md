...
so we strongly suggest to upgrade to WLS2 when possible,
...
10. Run the e3-build tool using your preferred specification
   ...
   where
      - `<builddir>` is a build directory you can create
      (e.g. `mkdir epicsbuild`) or is the same build directory you may have
      already created in point 6 if you run on WSL1
